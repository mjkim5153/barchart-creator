# 운항스케줄 바차트 표출 규칙

## 개요

`process_flight_schedule()`로 수집된 데이터를 기반으로 바차트 영역에 운항스케줄을 표출할 때 적용되는 규칙을 정의합니다.
데이터 컬럼 구조는 `ubkiais_rule.md` 참고.

---

## 1. 데이터 필터링

바차트에 표출하기 전 아래 조건으로 데이터를 필터링합니다.

| 조건 | 설명 |
|------|------|
| `Status != 'CNL'` | 결항편 제외 |
| `Bt_idx == 'Y'` | 중복 레코드 제거 (Y인 레코드만 표출) |

---

## 2. 항공기별 편 연결 규칙

- 같은 기번(`Acno`)에 속한 항공편을 **`RO_KST`(실적 Out) 오름차순**으로 정렬하여 순서대로 연결
  (`RO_KST`가 결측인 편은 순서 결정에 한해 `STD_KST`로 보완 — 그려지는 시각/BT/GT 값 자체는 여전히
  RO/RI 기준)
- 연결된 편들은 바차트에서 시간 흐름에 따라 이어서 표출

### 연결 무결성 검증

- **규칙**: 이전 편의 도착공항(`Arrstn`) = 다음 편의 출발공항(`Depstn`) 이어야 함
- **위반 시**: 해당 바차트에 **빨간색 테두리** 표시

```
[정상 연결]
편 A: ICN → GMP  →  편 B: GMP → PUS  ✓ (GMP == GMP)

[연결 오류 - 빨간 테두리]
편 A: ICN → GMP  →  편 B: PUS → ICN  ✗ (GMP ≠ PUS)
```

---

## 3. 바차트 모양 및 표출 정보

### 바차트 형태

- 직사각형 형태
- BAR 구간: `RO_KST`(실적 출발) ~ `RI_KST`(실적 도착) — 계획(`STD_KST`/`STA_KST`)이 아닌 실적 기준
- `RO_KST`/`RI_KST` 중 **하나만** 결측인 편은 STD/STA로 폴백하지 않고 바를 그리지 않으며,
  바차트 상단에 결측 경고 배너로 어떤 편(`Fltno`/`Acno`)의 무엇이 없는지 표출한다
- `RO_KST`/`RI_KST`가 **둘 다** 결측인 편은 `STD_KST`~`STA_KST`(계획)로 폴백해 바를 그린다 —
  실적 자체가 아예 없는 편(예: 미래 스케줄, 실적 미수집)까지 결측 경고 배너로 미표출 처리하면
  바차트가 지나치게 비어 보이는 문제가 있어, 부분 결측(연결 오류 가능성 있는 데이터 이상)과
  전체 결측(정상적으로 실적이 없는 상태)을 구분해 후자만 폴백 표출한다. 폴백 바는
  `.bar-planned`(반투명 + 점선 테두리)로 시각 구분해 실적 바와 혼동되지 않게 한다 — 이 편은
  결측 경고 배너 대상에서 제외된다(더 이상 "미표출"이 아니므로)
- (좌측 요약 영역의 가동률/GT 히스토그램/GT 테이블/노선별 BT 통계는 여전히
  `STD_KST`/`STA_KST`/`Blocktime` 계획 기준을 사용하며 이 절과 무관)

### 3-1. 지연 BAR (STD~RO)

- **조건**: `RO_KST > STD_KST` (실적 출발이 계획 출발보다 늦은 경우)만 표출
- **구간**: `STD_KST` ~ `RO_KST`
- **색상**: 붉은색 (`.bar-delay`, `#dc2626`), 메인 BAR(`RO_KST`~`RI_KST`) 왼쪽에 이어붙여 표출
- `RO_KST <= STD_KST`(정시 또는 조기출발)면 지연 BAR를 표출하지 않고, 기존과 동일하게
  `RO_KST`~`RI_KST` 메인 BAR만 표출
- 지연 BAR에는 편명/공항 라벨을 표시하지 않음(메인 BAR 라벨과 중복 방지)
- 구현 위치: `static/js/barchart.js`의 BAR 렌더링 루프(메인 BAR 직전), `fl.std_kst` 필드 사용
  (`data_processor.py::filter_for_barchart()`에서 `STD_KST` 컬럼을 isoformat 변환해 전달)

### 바차트 내부 표출 정보

```
    ┌──────────────────────────────┐
    │           KE123              │
    └──────────────────────────────┘
ICN                                 GMP
```

| 위치 | 표출 내용 |
|------|-----------|
| BAR 내부 중앙 | 편명 (`Fltno`) |
| BAR 하단 좌측 바깥 | 출발공항 (`Depstn`) |
| BAR 하단 우측 바깥 | 도착공항 (`Arrstn`) |

---

## 4. 바차트 배경색

| 조건 | 배경색 |
|------|--------|
| `Depstn == 'GMP'` 또는 `Arrstn == 'GMP'` | 파란색 |
| 그 외 | 주황색 |

---

## 4-1. STA(도착 예정시각) 마커

- 각 편의 BAR 내부(또는 조기도착 시 BAR 바깥)에 계획 도착시각(`STA_UTC`→KST 변환,
  `STA_KST`)을 **노란색 세로선**으로 표시
- 세로선 상단에 **노란색 정삼각형**(꼭짓점이 아래로 향해 선의 상단과 맞닿는 형태, 밑변이
  위) 표시
- 세로선 세로 범위: 해당 편 BAR의 높이(상단~하단)만큼만 — 다른 행(기번)과 겹치지 않음
- 삼각형 크기: BAR 높이(`scaledBarH`)에 비례 — Y축 확대/축소 시 함께 커지고 작아짐
- **조기도착**(실적 `RI_KST`가 계획 `STA_KST`보다 이른 경우) 시 마커가 BAR 오른쪽 바깥에
  위치할 수 있음 — 정상 동작이며 클리핑하지 않음(차트 전체 가로 영역 내에서는 항상 표시)
- 구현 위치: `static/js/barchart.js`의 BAR 렌더링 루프, `fl.sta_kst` 필드 사용
  (`data_processor.py::filter_for_barchart()`에서 `STA_KST` 컬럼을 isoformat 변환해 전달)

---

## 5. 바차트 사이 GROUND TIME 표시

- 연결된 두 바차트 사이의 **정 가운데**에 GROUND TIME을 텍스트로 표시
- GROUND TIME = 다음 편 `RO_KST` - 이전 편 `RI_KST`

```
예시:
│ KE123 ICN→GMP │  45분  │ KE456 GMP→PUS │
         RI_KST         RO_KST
```

> **참고**: "GT:" 접두어 없이 숫자+분만 표시
> **주의**: 이 바 사이 라벨은 기본적으로 **실제 GT**(다음 편 `RO_KST` - 이전 편 `RI_KST`, 아래
> 6번 마우스 오버 툴팁의 `ground_time_before`와 동일 값)를 표시한다. 확보 GT는 툴팁에서만
> 추가로 확인 가능 — 바 사이 라벨 자체는 세분화하지 않는다.
> **계획 GT 폴백**: 다음 편이 RO/RI 둘 다 결측이어서 STD/STA로 폴백 표출된 경우(3번 규칙)
> 실제 GT를 계산할 수 없으므로, 대신 **계획 GT**(`STD_KST` - 이전 편 `STA_KST`, `planned_gt`)를
> 실제 GT와 동일하게 `NN분` 형식(접미어 없음)으로 표시한다(`.gt-label-planned` 스타일 — 기울임 +
> 반투명으로만 시각 구분, 텍스트 자체는 구분하지 않음). 이때 이전 편의 바 끝점도 실제(`RI_KST`)가
> 있으면 그것을, 이전 편도 폴백 표출 중이면
> `STA_KST`를 기준으로 라벨 위치를 잡는다. 이전 편이 RO/RI 중 하나만 결측이라 바 자체가
> 그려지지 않는 경우(3번 규칙)는 기준점이 없으므로 GT 라벨도 표시하지 않는다.

---

## 5-1. 교대 행 배경색

- 기번 행의 배경색을 교대로 적용하여 가독성 향상
- 짝수행: transparent (투명)
- 홀수행: #d9d9d9 (연한 회색)
- **Y축 라벨 영역에도 동일한 교대 배경색 적용** (차트 영역과 일관성 유지)

---

## 5-1-1. 같은 기번 내 겹치는 편 lane 분리

### 배경

`RAMP RETURN`(지상 복귀) 등으로 같은 편명이 두 행으로 기록되는 경우가 있다(예: 2026-03-12
`HL8322` `7C6122` — 1행: KHH→KHH, RO 15:09~RI 15:40 / 2행: KHH→GMP, STD 15:15,
RO 16:53~RI 20:02). 2행의 **지연 BAR**(STD~RO 구간, 15:15~16:53)가 1행의 메인 BAR와
시간상 겹쳐, 같은 Acno 행 안에서 두 BAR가 겹쳐 그려지는 문제가 있었다.

### 규칙

`_sort_key`(RO_KST, 결측 시 STD_KST) 순서상 **바로 인접한 두 편만** 비교한다: 다음 편의
`STD_KST`가 이전 편의 `RI_KST`와 같거나 더 이르면(`다음 편 STD_KST <= 이전 편 RI_KST`)
겹침으로 판정하고, 그 Acno 행 안에서 두 편을 위/아래 lane(0/1 토글)으로 나눠 배치한다.
겹치지 않으면 lane 0으로 리셋해 기존과 동일하게 한 줄에 이어붙인다.

- 각 편의 **BAR 두께는 그대로 유지**하고, 겹침이 있는 Acno만 행 전체 높이가
  `lane_count`배로 늘어난다(다른 Acno 행 높이·교대 배경·Y축 번호는 영향 없음).
- lane은 순수 렌더링 배치용 정보로, 기존 렌더링 순서·GT(`ground_time_before`) 계산·
  `connection_error`(이전 편 Arrstn vs 다음 편 Depstn) 판정에는 영향을 주지 않는다.
- 구현 위치: `data_processor.py::_assign_overlap_lanes()`(lane/lane_count 산출),
  `static/js/barchart.js`의 `redrawAll()`(행별 높이 누적합 `rowOffsets`/`rowHeights`
  기반으로 Y좌표 계산 — 행 배경, Y축 라벨, BAR, 스크롤 총 높이, 드래그 재배정 좌표→행
  인덱스 역산이 모두 이 배열을 참조하도록 통일됨)

---

## 5-2. Y축 넘버링

- Y축 라벨에 순번을 표시: `1 HL7234`, `2 HL7235` 형식
- MARGIN.left를 110px로 확장하여 번호 공간 확보

---

## 5-3. 줌/스크롤 동작

| 입력 | 동작 |
|------|------|
| 마우스 휠 | Y축 스크롤 (세로 이동) |
| Ctrl+휠 / 핀치 | X, Y 동시 줌 |

### X축 범위 제한
- X축 범위는 검색 조건(시작일/종료일)이 아니라 **실제로 표출되는 항공편의 RO_KST/RI_KST 최소~최대값**으로 결정됨 (`static/js/barchart.js`에서 데이터 기반으로 산출). RO_KST/RI_KST가 결측인 편은 각각 STD_KST/STA_KST로 폴백해 범위에 포함시킨다(위 3번 규칙의 "둘 다 결측 → STD/STA 폴백 표출"과 동일 기준 — 그렇지 않으면 RO/RI가 전부 결측인 데이터셋에서 X축 범위가 산출되지 않아 폴백 BAR까지 전부 화면 밖으로 밀려 안 보이게 된다)
- 단일일 검색이라도 야간편(예: 장거리 국제선)의 실제 출발(RO)이 검색일 전날 저녁이면 그 전날 저녁부터 바가 시작되도록 X축이 자연 확장됨 — 검색일 00:00~24:00로 강제로 압축하지 않음
- 다일 시 각 날짜의 00:00 위치에 "M/D" 형식 날짜 라벨 표시
- 날짜 경계선은 파란색 점선으로 표시

### 방향키 스크롤
- ArrowLeft/Right: 수평 스크롤

### 키보드 줌
- Ctrl+ArrowRight: X축 확대
- Ctrl+ArrowLeft: X축 축소
- Ctrl+ArrowUp: Y축 확대
- Ctrl+ArrowDown: Y축 축소

### 축소 시 텍스트 숨김
- ROW_HEIGHT * yZoom < 30px이면 BAR 내부 텍스트(편명, 공항, GT) 숨김
- 마우스 오버 툴팁은 축소 상태에서도 항상 표시

---

## 6. 마우스 오버 툴팁

바차트에 마우스 오버 시 아래 정보를 툴팁으로 표시합니다.

| 항목 | 컬럼 |
|------|------|
| 편명 | `Fltno` |
| 기번 | `Acno` |
| 출발공항 | `Depstn` |
| 도착공항 | `Arrstn` |
| 실적 출발시각(RO) | `RO_KST` |
| 실적 도착시각(RI) | `RI_KST` |
| 블록타임 | `RI_KST` - `RO_KST` (분, 계획 `Blocktime`과 별개) |
| 계획 GT | 해당 편 `STD_KST` - 이전 편 `STA_KST` (분) |
| 확보 GT | 해당 편 `STD_KST` - 이전 편 `RI_KST` (분) |
| 실제 GT | 해당 편 `RO_KST` - 이전 편 `RI_KST` (분, `ground_time_before`와 동일 — 바 사이 라벨과 같은 값) |
| 연결지연 / 출발지연 / 관제지연 | `CNX_DLA` / `DEP_DLA` / `ATC_DLA` (분, 값 없으면 `-`) |
| 지연(RO-STD) | `RO_KST` - `STD_KST` (분, `RO_KST > STD_KST`일 때만 계산, 아니면 `-`) — 지연 BAR와 동일 조건, 프론트(`static/js/barchart.js`)에서 계산 |
| NAT | 자유 텍스트, 엑셀 업로드 필수 입력 컬럼(값 없으면 `-`) — 필터/집계 미사용, 툴팁 표시 전용 |
| MTTT | 정수, 엑셀 업로드 필수 입력 컬럼(값 없으면 `-`) — 필터/집계 미사용, 툴팁 표시 전용 |

> **GROUND TIME 세분화 (계획/확보/실제)**:
> - **계획 GT**: 스케줄상 원래 확보된 여유 — 해당 편 `STD_KST` - 이전 편 `STA_KST`
> - **확보 GT**: 이전 편이 실제 도착한 시점 기준 계획 출발까지 남은 여유 — 해당 편 `STD_KST` -
>   이전 편 `RI_KST`
> - **실제 GT**: 실적 기준 최종 여유 — 해당 편 `RO_KST` - 이전 편 `RI_KST` (기존 방식과 동일,
>   `data_processor.py`의 `ground_time_before` 키)
> - 세 값 모두 STD/STA/RO/RI 중 필요한 값이 결측이면 해당 값만 개별적으로 `-` 처리 (다른 GT
>   계산에는 영향 없음)
> - 첫 번째 편(앞에 연결된 편 없음)은 세 GT 모두 미표시(`-`)
> - 구현 위치: `data_processor.py::filter_for_barchart()`에서 `planned_gt`/`secured_gt`/
>   `ground_time_before` 계산, `static/js/barchart.js`의 툴팁 렌더링에서 세 행으로 표출

> **D_OPER 컬럼**: 엑셀 업로드 필수 컬럼(값은 `Y` 또는 빈칸만 허용)으로 받되, 바차트
> 표출(BAR/툴팁 등)에는 사용하지 않는다 — 업로드 데이터 보관 전용.

---

## 규칙 요약

```
[데이터 필터] Status != CNL  AND  Bt_idx == Y
      ↓
[정렬] 기번(Acno) 별로 RO_KST 오름차순 (결측 시 STD_KST로 순서만 보완)
      ↓
[연결 검증] 이전 Arrstn == 다음 Depstn
           → 불일치 시 빨간 테두리
      ↓
[배경색] GMP 포함 시 파란색 / 그 외 주황색
      ↓
[BAR 표출] 출발공항 | 편명 | 도착공항  (구간: RO_KST ~ RI_KST, 결측 시 미표시 + 경고 배너)
      ↓
[사이 텍스트] GROUND TIME (분, RI_KST~RO_KST 기준)
      ↓
[마우스 오버] 편명, 기번, 출도착공항, RO/RI_KST, BT, GT, 지연 3종(CNX/DEP/ATC),
             지연(RO-STD), NAT, MTTT
```

> **요약 영역과의 관계**: 좌측 요약 패널(가동률 "계획BT", GT 히스토그램, GT 테이블, 노선별 BT 통계)은
> 이 문서의 RO/RI 전환과 무관하게 `STD_KST`/`STA_KST`/`Blocktime`(계획) 기준을 그대로 사용한다.
