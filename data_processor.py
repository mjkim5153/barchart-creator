...
_grade_map = {
    'A': ['AT7', 'DH8', 'E19', 'CRJ'],
    'B': ['E75', 'E90', 'BCS'],
    'C': ['B737', 'B738', 'B739', 'B38M', 'A319', 'A320', 'A321', 'A20N', 'A21N'],
    'D': ['B767', 'B787', 'B788', 'B789', 'A330', 'A332', 'A333'],
    'E': ['B777', 'B772', 'B773', 'A350', 'A359', 'B747', 'B748', 'A380'],
}
...
def process_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # #10 Actype_Grade
    df['Actype_Grade'] = df['Actype'].map(ACTYPE_GRADE).fillna('')
...
def filter_for_barchart(df: pd.DataFrame, airline: str, nat_list: list) -> dict:
    ...
    for acno, group in filtered.groupby('Acno'):
        ...
        for i, row in group.iterrows():
            ...
            color = 'blue' if (dep == 'GMP' or arr == 'GMP') else 'orange'
...
def compute_summary(df: pd.DataFrame, nat_list: list, airline: str = '') -> dict:
    ...
    # 일별 운영기재수 합산 방식으로 가동률 계산
    op_rows = []
    for al_code in sorted(DOMESTIC_AIRLINES):
        al_df = all_pax[all_pax['Airline_Code'] == al_code]
        if al_df.empty:
            continue
    ...
    for date_val in available_dates:
        ...
        day_rows = []
        for al_code in sorted(DOMESTIC_AIRLINES):
            al_df = day_pax[day_pax['Airline_Code'] == al_code]
            if al_df.empty:
                continue
            op_aircraft = al_df['Acno'].nunique()
            pl_bt = int(al_df['Blocktime'].fillna(0).sum())
            hours_per = round(pl_bt / op_aircraft / 60, 1) if op_aircraft > 0 else 0.0
            day_rows.append({
                'airline': al_code,
                'operating_aircraft': float(op_aircraft),
                'planned_bt': pl_bt,
                'hours_per_aircraft': hours_per,
            })
        daily_op[str(date_val)] = day_rows
